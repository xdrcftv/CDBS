...
dir_path = 'C:/Users/admin/PycharmProjects/CDBS/ARAA_analysis/ARAA_13C'
...
for n, file in enumerate(CDBS_files):
    ...
    if n == 0:
        ...
        y_err = np.sqrt(y_avg_ref)
        y_normal = np.divide(y_avg_ref, np.max(y_avg_ref))
        y_normal_err = np.divide(y_err, np.max(y_avg_ref))

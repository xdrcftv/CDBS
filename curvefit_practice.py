...
CDBS_path = 'C:/Users/admin/Desktop/CDBS/'
subdir_name = []
subdir_path = []
...
for subdir in subdir_path:
    CDBS_matrix.append(asc_parser(subdir))
...
for i, matrix in enumerate(CDBS_matrix):
    ...
    y_hr = gaussian(x_hr, 1, 0, wid=result.best_values['wid'])

    # AUC = integrate.simps(result.best_fit, x)
    # plt.plot(np.divide(result.best_fit, AUC), label=subdir_name[i])

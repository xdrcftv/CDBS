...
for n, file in enumerate(CDBS_files):
    ...
    AUC_center = integrate.simps(y_CDBS[width-2:width+2], x_CDBS[width-2:width+2])
    ...
    plt.errorbar(x_adj_CDBS, y_normal, yerr=y_normal_err, fmt=linestyle[n], label=label_list[n], capsize=3)
plt.title('PET CDBS')
plt.yscale('log')
